...
contract RevoAirdropNFT is Ownable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public nfts;//Index => index , nftId
    ...
    function addNFT(uint256 _index, uint256 _tokenId) public onlyOwner {
        enqueuePendingTx(_index, _tokenId);
    }
    ...
    */
    ...
    function enqueuePendingTx(uint256 _index, uint256 _tokenId) private {
        lastPending[_index] = lastPending[_index].add(1);
        nfts[_index][lastPending[_index]] = _tokenId;
    }

    function dequeuePendingTx(uint256 _index) private returns (uint256 data) {
        require(lastPending[_index] >= firstPending[_index], "Empty queue");  // non-empty queue

        data = nfts[_index][firstPending[_index]];

        delete nfts[_index][firstPending[_index]];
        firstPending[_index] = firstPending[_index].add(1);
    }
    ...
    function withdrawNFTs(uint256[] memory _tokenIds) public onlyOwner {
        for(uint256 i = 0; i < _tokenIds.length; i ++){
            revoNFT.transferFrom(address(this), owner(), _tokenIds[i]);
        }
    }
}
